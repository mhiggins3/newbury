...
object Discovery {
	case class Discovery(id: Option[Long] = None, uuid: String = java.util.UUID.randomUUID().toString(), time: Date, beaconId: Long, userId: Long, goalId: Long) extends Entity
	...
	class Discoveries(tag: Tag) extends EntityTable[Discovery](tag, "DISCOVERY"){
	  ...
	  def * = (id.?, uuid, time, beaconId, userId, goalId) <> (Discovery.tupled, Discovery.unapply)
